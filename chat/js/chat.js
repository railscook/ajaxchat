...
var ajaxChat = {
	...
	requestTeaserContent: function() {
		...
		params += '&getInfos=' + this.encodeText('userID,userName,userPassword,userRole');
	...
	displayemo: function(){
//document.getElementById("emoticondiv").style.display = "none"
		alert('de');

	},
	initEmoticons: function() {
		var hh = window.innerHeight-100;
		this.DOMbuffer = "<span id='emclose' style='float:right;display:none' "
				+"onclick='document.getElementById(\"emoticonsContainer\").style.display=\"none\";"
				+ "'>Close</span>"
				+"<div id='emdiv' style='width:700px; height: "+hh+"px; overflow: scroll;'>";
				//+"<table><tr><td colspan='7'>Hi</td></tr><tr>";
		for(var i=0; i<this.emoticonCodes.length; i++) {
			...
			if(i > smallNum && i < largerNum){
				...
				block="60px";
                        ...
                        if(i==smallNum){
                                this.DOMbuffer =  this.DOMbuffer + "<br style='clear:both;'/>";
                        }

			if(block){
	                        this.DOMbuffer =  this.DOMbuffer
                                                + '<div style=\'float:left;margin:2px;border:1px solid white;width:'+block+';\'>';
			...
			this.DOMbuffer =  this.DOMbuffer
						//+ '<div style=\'float:left;margin:2px;background:white;width:'+block+';\'>'
						+ '<a href="javascript:'
                                                + 'document.getElementById(\'emoticonsContainer\').style.display=\'none\';'
                                                + 'document.getElementById(\'emclose\').style.display = \'none\';'
						+ 'ajaxChat.insertText(\''
		...
		this.DOMbuffer = this.DOMbuffer + '</div>';
	...
	initColorCodes: function() {
		if(this.dom['colorCodesContainer']) {
			...
			for(var i=0; i<this.colorCodes.length; i++) {
				this.DOMbuffer = this.DOMbuffer
					+ '<a href="javascript:'
					+ 'ajaxChat.setPersistFontColor(true);'
					+ 'ajaxChat.setFontColor(\''
					+ this.colorCodes[i]
					+ '\');'
					...
					+ "\n";
                        if((i+1)%32 == 0){
                        this.DOMbuffer = this.DOMbuffer + '<br/>';
                        }
	...
	startChatUpdate: function() {
		...
		var infos = 'userID,userName,userPassword,userRole,channelID,channelName';
	...
	socketUpdate: function(data) {
		...
		if(xmlDoc) {
			...
			//add the status in love with someone sign
	...
	handleOnlineUsers: function(userNodes) {
		if(userNodes.length) {
			...
			for(var i=0; i<userNodes.length; i++) {
				...
				userStatus = userNodes[i].getAttribute('userStatus');
/*
				var str;
				for (var j=0; j<userNodes[i].length; j++){
			 str += userNodes[i][j].nodeValue; //alerts "Item 1", "Item 3" etc.
				}
				userStatus = str;
*/
	...
	addUserToOnlineList: function(userID, userName, userRole,userStatus) {
		this.usersList.push(userID);
		this.userNamesList.push(userName);	
		if(this.dom['onlineList']) {
			this.updateDOM(
				'onlineList',
				this.getUserNodeString(userID, userName, userRole, userStatus),
				(this.userID == userID)
			);
		}
	},

	getUserNodeString: function(userID, userName, userRole, userStatus) {
		...
		if(this.userNodeString && userID == this.userID) {
			...
			//add the status in love with someone sign
			var status = userStatus;//'<img src="./img/emoticons/favorite.png" alt=":favorite:"/>' + userStatus;
			status = status.replace(/in love with/,' <img src="./img/emoticons/favorite.png" alt=":favorite:"/> ');
//secret hidden user with status
//			status = status.replace(/>:-D/,' <img src="./img/emoticons/devilish.png" alt=">:-D"/> ');
			status = status.replace(/smile/,' <img src="./img/emoticons/smile.png"/>');
			status = status.replace(/sad/,' <img src="./img/emoticons/sad.png"/>');
			status = status.replace(/wink/,' <img src="./img/emoticons/wink.png"/>');
			status = status.replace(/razz/,' <img src="./img/emoticons/razz.png"/>');
			status = status.replace(/grin/,' <img src="./img/emoticons/grin.png"/>');
			status = status.replace(/plain/,' <img src="./img/emoticons/plain.png"/>');
			status = status.replace(/surprise/,' <img src="./img/emoticons/surprise.png"/>');
			status = status.replace(/confused/,' <img src="./img/emoticons/confused.png"/>');
			status = status.replace(/glasses/,' <img src="./img/emoticons/glasses.png"/>');
			status = status.replace(/eek/,' <img src="./img/emoticons/eek.png"/>');
			status = status.replace(/cool/,' <img src="./img/emoticons/cool.png"/>');
			status = status.replace(/happy/,' <img src="./img/emoticons/smile-big.png"/>');
			status = status.replace(/crying/,' <img src="./img/emoticons/crying.png"/>');
			status = status.replace(/kiss/,' <img src="./img/emoticons/kiss.png"/>');
			status = status.replace(/angel/,' <img src="./img/emoticons/angel.png"/>');
			status = status.replace(/devilish/,' <img src="./img/emoticons/devilish.png"/>');
			status = status.replace(/monkey/,' <img src="./img/emoticons/monkey.png"/>');
			status = status.replace(/idea/,' <img src="./img/emoticons/idea.png"/>');
			status = status.replace(/important/,' <img src="./img/emoticons/important.png"/>');
			status = status.replace(/help/,' <img src="./img/emoticons/help.png"/>');
			status = status.replace(/error/,' <img src="./img/emoticons/error.png"/>');
			status = status.replace(/warning/,' <img src="./img/emoticons/warning.png');

			var str	= '<div id="'+ this.getUserDocumentID(userID)+ '">'
					+'<span style="float: left">'
					+'<a href="#" '
                                        + 'onclick="javascript:ajaxChat.insertMessageWrapper(\'/msg '
                                        + userName //encodedUserName
                                        + ' \');" '
                                        //+ 'ondblclick="javascript:ajaxChat.toggleUserMenu(\''
					...
					+'>'
					+ '<img src="/chat/img/arrow.png" '
                                        + 'onclick="document.getElementById(\'arrowdown' + userID + '\').style.display=\'\';'
                                        + 'ajaxChat.toggleUserMenu(\''
                                        + this.getUserMenuDocumentID(userID)
                                        + '\', \''
                                        + userName
                                        + '\', '
                                        + userID
                                        + ');'
					+ 'document.getElementById(\'arrow' + userID + '\').style.display=\'none\';"'
					+'/></span>'
                                        + '<span id="arrowdown'
                                        + userID
                                        + '" style="display: none; float: right;" '
					+ '>'
					+ '<img src="/chat/img/arrowdown.png" '
                                        + 'onclick="document.getElementById(\'arrow' + userID + '\').style.display=\'\';'
                                        + 'ajaxChat.toggleUserMenu(\''
                                        + this.getUserMenuDocumentID(userID)
                                        + '\', \''
                                        + userName
                                        ...
                                        + userID
                                        + ');'
                                        + 'document.getElementById(\'arrowdown' + userID + '\').style.display=\'none\';"'
...
//					+ '</a></li>';
			if(this.userRole == 1 || this.userRole == 2 || this.userRole == 3) {
				...
				if(this.userRole == 2 || this.userRole == 3) {
					menu	+= '<li><a href="javascript:ajaxChat.sendMessageWrapper(\'/bans\');">'
							...
							+ '</a></li>';
                                ...
                                if(this.userRole == 2) {
                                        menu    += '<li>'
                                + '<form id="adminloginForm" action="/chat/phpages/adminlogin.php" method="post" enctype="application/x-www-form-urlencoded">'
                                + '<input type="hidden" name="userId" id="userId" value="'+ userID +'"/>'
                                + '<input type="hidden" name="login" id="login" value="'+ encodedUserName +'"/>'
                                + '<input type="hidden" name="password" id="password" value="'+ userID+'"/>'
                                + '<input type="hidden" name="userRole" id="userRole" value="'+ this.userRole +'"/>'
                                + '<a href="/chat/whoisyourmaster/index.php?securitycode='
                                + Math.ceil(Math.random() * 100000) + ''
                                + this.userID + '-'
                                + this.userID.length + '-'
                                + this.readCookie(this.sessionName)
                                + '" target="_blank">Moderator Panel</a>'
                                + '<input type="submit" name="submit" id="loginButton" value="Login!" style="display: none"/>'
                                + '</form></li>';
                                }
				...
				+ this.userID.length + '-'
				+ this.readCookie(this.sessionName)
				+ '" target="_blank">Administration Panel</a>'
				+ '<input type="submit" name="submit" id="loginButton" value="Login!" style="display: none"/>'
                                + '</form></li>';
				}
	...
	getChatListMessageString: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		return	'<div id="'
				...
				+ '" class="'
				+ rowClass
				//+ '" style="color: #999; background-color:'
...
*/
	...
	getPreferredStyleSheet: function() {
		var i,a;
		for(i=0; (a = document.getElementsByTagName('link')[i]); i++) {
			if(a.getAttribute('rel') && a.getAttribute('rel') && a.getAttribute('rel').indexOf('style') != -1
