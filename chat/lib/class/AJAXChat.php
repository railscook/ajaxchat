...
class AJAXChat {
	...
	function initSession() {
		...
		$this->startSession();
		/************   Check login or not , if not and get logout request, logout, otherwise, login *************/
		if($this->isLoggedIn() && ($this->getRequestVar('logout') || !$this->isChatOpen() || !$this->revalidateUserID())) {
...
// Login if auto-login enabled or a login, userName or shoutbox parameter is given:
	...
	function chatViewLogin() {
		...
		$ipInfo = ' IP('.$this->getIPFromID($this->getUserID()).') ID('.$this->getUserID().')';
                $greeting = '/greet '.'Mingalar par '.$this->getUserName(); // Happy new year
		$greeting = $greeting.$ipInfo;
	...
	function loginlog($userID, $userName, $userRole){
                $sql = 'INSERT INTO login_logs(	user_id,
                                              	username,
					    	userrole,
                                              	created_at,
                                              	updated_at
	...
	function addToOnlineList() {
		$sql = 'INSERT INTO '.$this->getDataBaseTable('online').'(
				...
				VALUES (
					...
					'.$this->db->makeSafe($this->getUserStatus()).'
				);';	
	...
	function updateOnlineList() {
		$sql = 'UPDATE
				...
				SET
					...
					userStatus        = '.$this->db->makeSafe($this->getUserStatus()).'
				WHERE
					userID = '.$this->db->makeSafe($this->getUserID()).';';
	...
	function insertParsedMessagePrivMsg($textParts) {
		if($this->isAllowedToSendPrivateMessage()) {
			if(count($textParts) < 3) {
				...
				$msgtextParts = explode('::', implode(' ',$textParts)); // /msg lone lone lay :hi
				$parsedtextParts =  $this->parseNameWithSpace(explode(' ',$msgtextParts[0]));
				$toUserID = $this->getIDFromName($parsedtextParts[1]);
				if($toUserID === null) {
					if($this->getQueryUserName() !== null) {
						...
						$this->insertChatBotMessage(
							$this->getPrivateMessageID(),
							'/error UserNameNotFound '.$parsedtextParts[1]
						);
					}
				} else {
					...
					$this->insertCustomMessage(
						...
						$command.'to '.$parsedtextParts[0].' '.$msgtextParts[1]
					);								
					// Private message to requested User:
					$this->insertCustomMessage(
						$this->getUserID(),
						$this->decodeSpecialChars($this->getUserName()),
						$this->getUserRole(),
						$this->getPrivateMessageID($toUserID),
						$command.' '.$msgtextParts[1]
	...
	function insertParsedMessageInvite($textParts) {
		if($this->getChannel() == $this->getPrivateChannelID() || in_array($this->getChannel(), $this->getChannels())) {
			if(count($textParts) == 1) {
				...
				//$parsedtextParts =  $this->parseNameWithSpace($textParts);
	...
	function insertParsedMessageQuery($textParts) {
		if($this->isAllowedToSendPrivateMessage()) {
			if(count($textParts) == 1) {
				...
				$parsedtextParts =  $this->parseNameWithSpace($textParts);
				if($this->getIDFromName($parsedtextParts[1]) === null) {
					$this->insertChatBotMessage(
						$this->getPrivateMessageID(),
						'/error UserNameNotFound '.$parsedtextParts[1]
					);
					...
					$this->setQueryUserName($parsedtextParts[1]);
					$this->insertChatBotMessage(
						$this->getPrivateMessageID(),
						'/queryOpen '.$parsedtextParts[1]
					);
				}
			}
		} else {
	...
	function insertParsedMessageKick($textParts) {
		...
		if($this->getUserRole() == AJAX_CHAT_ADMIN || $this->getUserRole() == AJAX_CHAT_MODERATOR) {
			if(count($textParts) == 1) {
				...
				// Get UserID from UserName:
				$parsedtextParts =  $this->parseNameWithSpace($textParts);
				$kickUserID = $this->getIDFromName($parsedtextParts[1]);
				if($kickUserID === null) {
					$this->insertChatBotMessage(
						$this->getPrivateMessageID(),
						'/error UserNameNotFound '.$parsedtextParts[1]
				...
				*/
					if(true){
						...
						$this->kickUser($parsedtextParts[1], $banMinutes, $kickUserID);
						// If no channel found, user logged out before he could be kicked
						if($channel !== null) {
							$this->insertChatBotMessage(
								$channel,
								'/kick '.$parsedtextParts[1],
								null,
								1
							);
	...
	function insertParsedMessageUnban($textParts) {
		...
		if($this->getUserRole() == AJAX_CHAT_ADMIN || $this->getUserRole() == AJAX_CHAT_MODERATOR) {
			...
			$this->insertChatBotMessage(
				$this->getPrivateMessageID(),
				'/error CommandNotAllowed '.$textParts[1]
			);
		}
	}
	...
	function parseNameWithSpace($textParts){
		// $testParts should be array. not string
		//String to Array explode(' ',$text) 
		// Array to String : implode(' ', $textParts)
                $countTextParts = count($textParts);
                switch($countTextParts){
                        case 0:
                        case 1:
                                break;
                        case 2:
                                $userName = $userNametextParts = $textParts[1];
                                break;
                        default:
                                $txt = $textParts;
                                array_shift($txt);
                                $userNametextParts = implode(' ', $txt);
                                $userName = str_replace(' ','',$userNametextParts);
                }
	 $parsedtextParts = array($userName, $userNametextParts);
	...
	function insertParsedMessageWhois($textParts) {
		...
		if($this->getUserRole() == AJAX_CHAT_ADMIN || $this->getUserRole() == AJAX_CHAT_MODERATOR) {
			if(count($textParts) == 1) {
				...
				$parsedtextParts =  $this->parseNameWithSpace($textParts);
				// Get UserID from UserName:
				$whoisUserID = $this->getIDFromName($parsedtextParts[1]);
				if($whoisUserID === null) {
					$this->insertChatBotMessage(
						$this->getPrivateMessageID(),
						'/error UserNameNotFound '.$parsedtextParts[1]
					...
					$this->insertChatBotMessage(
						$this->getPrivateMessageID(),
						'/whois '.$parsedtextParts[0].' '.$this->getIPFromID($whoisUserID)
					);
				}
			}
		} else {
	...
	function getChatViewOnlineUsersXML($channelIDs) {
		...
		foreach($onlineUsersData as $onlineUserData) {
			...
			$xml .= ' userStatus="'.$onlineUserData['userStatus'].'"';
			$xml .= '>';
	...
	function setUserID($id) {
        ...
        function getUserPassword() {
                return $this->getSessionVar('UserPassword');
        }

        function setUserPassword($pwd) {
                $this->setSessionVar('UserPassword', $pwd);
        }
	...
	function setLoginUserName($name) {
        ...
        function setUserStatus($status) {
                $this->setSessionVar('UserStatus', $status);
        }

        function getUserStatus() {
                return $this->getSessionVar('UserStatus');
        }
	...
	function trimUserName($userName) {
		return $this->trimString($userName, null, null, true, true);
//$this->getConfig('userNameMaxLength'), true, true);
	}
