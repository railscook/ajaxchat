...
class AJAXChatTemplate {
			...
			case 'STYLE_OPTIONS':
				return $this->getStyleOptionTags();
                        case 'WEBSITE_OPTIONS':
                                return $this->getWebsiteOptionTags();
	...
	function getChannelOptionTags() {
        ...
        function getWebsiteOptionTags() {
                $websiteOptions = '';
                foreach($this->ajaxChat->getConfig('websiteAvailable') as $websites) {
			$website = $websites[0];
			$link = $websites[1];
                        $selected = ($website == $this->ajaxChat->getConfig('websiteDefault')) ? ' selected="selected"' : '';
                        $websiteOptions .= '<option value="'.$this->ajaxChat->htmlEncode($link).'"'.$selected.'>'.$this->ajaxChat->htmlEncode($website).'</option>';
                }
